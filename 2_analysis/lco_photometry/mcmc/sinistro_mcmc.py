...
COMMONPATH = '../../../data/lco_aumic/folded_sinistro/'
paths = sorted([os.path.join(COMMONPATH, specific) for \
                specific in os.listdir(COMMONPATH) if specific.endswith('.csv')])
...
def main():
    ...
    with open(pathlist[4], "rb") as gfilt:
        ...
        for i in range(ndim):
            ...
            lister2.append(p50)
        a1_g, a2_g = lister2
    with open(pathlist[6], "rb") as rfilt:
        ...
        for i in range(ndim):
            ...
            lister3.append(p50)
        a1_r, a2_r = lister3
    with open(pathlist[5], "rb") as ifilt:
    ...
    model_avg = 0
    cons_guess = 0
    for filt, data, alister in zip(['g', 'r', 'i'], [G_FOLD, R_FOLD, I_FOLD], [[a1_g, a2_g], [a1_r, a2_r], [a1_i, a2_i]]):
        ...
        def logprior(theta):
            ...
            for i in range(len(theta)):
                if i==2 and (theta[i] > const_low and theta[i] < constant_upper):
                    pass
                elif theta[i] is not None and not np.isnan(theta[i]):
    ...
    print(f'Model {num} average difference total in all 3 filters is: {model_avg}, with an average of {model_avg/3}\n')
main()
